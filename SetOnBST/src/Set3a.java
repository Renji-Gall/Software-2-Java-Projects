    ...
    private static <T extends Comparable<T>> T removeFromTree(BinaryTree<T> t, T x) {
        ...
        if (root.compareTo(x) == 0) {
            ...
            if (right.size() > 0 && left.size() == 0) {
                ...
                T small = removeSmallest(right);
                t.assemble(small, left, right);
...
private static <T> boolean satisfiesHeapOrdering(BinaryTree<Integer> t) {
    if (t == null || t.size() == 0) {
        return true;
    }
    ...
    t.disassemble(root, left, right);

    if (left != null && !left.size() == 0) {
        if (root > left.root() || !satisfiesHeapOrdering(left)) {
            return false;
        }
    }

    if (right != null && !right.size() == 0) {
        if (root > right.root() || !satisfiesHeapOrdering(right)) {
            return false;
        }
    }
