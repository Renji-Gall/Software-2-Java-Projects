...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final Pair<K, V> remove(K key) {
        ...
        Map.Pair<K, V> remove = this.hashTable[bucket].remove(key);
        this.size--;
    ...
    public final Pair<K, V> removeAny() {
        ...
        while (this.hashTable[remove].size() == 0) {
            remove++;
        }

        Map.Pair<K, V> removed = this.hashTable[remove].removeAny();
        this.size--;
    ...
    public final boolean hasKey(K key) {
        ...
        for (int i = 0; i < length; i++) {
            for (Pair<K, V> pair : this.hashTable[i]) {
