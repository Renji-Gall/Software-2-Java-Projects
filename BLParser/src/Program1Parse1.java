...
public final class Program1Parse1 extends Program1 {
    ...
    private static String parseInstruction(Queue<String> tokens, Statement body) {
        ...
        Reporter.assertElseFatalError(Tokenizer.isIdentifier(identifier),
                "Error: Is not valid identifier");

        String is = tokens.dequeue();
        Reporter.assertElseFatalError(is.equals("IS"), "Error: Expected IS");

        body.parseBlock(tokens);

        String end = tokens.dequeue();
        Reporter.assertElseFatalError(end.equals("END"), "Error: Expected END");

        String endName = tokens.dequeue();
        Reporter.assertElseFatalError(endName.equals(identifier),
                "Error: Instruction names between start and finish don't match");
