...
public Sequence<Integer> generatedCode() {
    ...
    Map<String, Statement> cCopy = new Map1L<>();
    for (Map.Pair<String, Statement> p : this.context) {
        cCopy.add(p.key(), p.value());
    }

    Statement bCopy = this.body.newInstance();
    this.body.transferFrom(bCopy);

    Sequence<Integer> cp = new Sequence1L<>();

    generateCodeForStatement(bCopy, cCopy, cp);
